...
class Products{
    async getProducts(){
        try{
            let result = await fetch("https://628969ede5e5a9ad32190086.mockapi.io/products");
...
class UI{
        ...
        buttons.forEach(button =>{
            ...
            if(inCart){
                ...
                button.addEventListener("click", event =>{
                    ...
                    //*sepetteki arttır/azalt kısmı
                    this.saveCartValues(cart);
                    //*sepete ekleme
                    this.addCartItem(cartItem);
                    //*sepete eklenince açılacak
                    this.showCart();
    ...
    addCartItem(item){
        ...
        <div class="cart-left-info">
            <a class="cart-left-info-title" href="#">${item.title}</a>
             <span class="cart-left-info-price">${item.price} TL</span>
        </div>
    </div>
    ...
    setupAPP(){ //*sayfa yüklendiğinde olur
        cart = Storage.getCart();
        this.saveCartValues(cart);
        this.populateCart(cart); //*sepete yükleme sırası 
    }
    populateCart(cart){
